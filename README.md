...
A token is a string of cryptic text that is provided by the server and stored on the client side using local storage or session storage to authenticate users who attempt to log in to a server, a network or some other secure systems.

- [ ] What steps can you take in your web apps to keep your data secure?

We can authenticate our web apps in order to keep our data in web apps secure, as this ensures that the relevant information we store in our web apps can only be accessed by those with the necessary authentication credentials.

- [ ] Describe how web servers work.

The term web server refers to both the computer that stores the code for a website and the program that runs on such a computer. Web servers work by dispensing the relevant information or resources as requested by a web browser over an HTTP request.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

The POST, GET, PUT and DELETE HTTP request methods can be mapped respectively to the Create, Read, Update and Delete parts in the CRUD acronym that we use when interfacing with APIs/Servers.
